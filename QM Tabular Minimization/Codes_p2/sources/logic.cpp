...
void Logic::L_NOT(int to1, int to0, wire* output, vector<wire*> inputs)
{
	gates.push_back(new NOT(to1, to0, inputs, output));
}
...
vector<string> Logic::calculate(string input_values)
{
	for(int i = 0; i < gates.size(); i++)
		for(int j = i + 1; j < gates.size(); j++)
			if(gates[i]->inputs == gates[j]->inputs)
				if(gates[i]->output == gates[i]->output)
