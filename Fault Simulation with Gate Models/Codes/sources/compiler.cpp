...
void Compiler::compile()
{
	...
	for (auto line:sv_lines)
	{
		...
		else if (tokens[KEYWORD_INDEX] == NOT)
			form_gate(tokens[DATA_INDEX], NOT, line_num);	
		else if (tokens[KEYWORD_INDEX] == END_MODULE)
			if(tokens.size() > ONE || sv_lines.size() > line_num )
